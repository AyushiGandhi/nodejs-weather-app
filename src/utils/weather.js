...
let weather = (address ,callback) =>
{
  ...
  req({url,json:true},(reject,response)=>{
    if (reject){callback({error : 'Unable to connect to the Server'})}
    else if(!response.body.request){callback({ error : 'Please Check The Name of The Address : Invalid Data '})}
